...
public:
	...
	const vector<Float>& get_prob_bulge() const;
	const vector<Float>& get_prob_exterior() const;
	const vector<Float>& get_prob_hairpin() const;
	const vector<Float>& get_prob_internal() const;
	const vector<Float>& get_prob_multiloop() const;
	...
	void debug_raccess_outer_branch(int i, int j) const;
	...
	void set_debug_beam(int j0, int j1, int interval = 1, bool stop_after = false);
	void set_debug_multi_energy(int i, int j);
	void set_debug_outer_alpha(int max_hits = 50);
	void set_debug_outer_dp_inside(int i0, int i1);
	void release_energy(bool leak);
	~LinCapR();
	void set_debug_skip_outside(bool enable);
	void set_debug_skip_profile(bool enable);
...
private:
	...
	bool debug_beam_log = false;
	int debug_beam_j0 = -1;
	int debug_beam_j1 = -1;
	int debug_beam_interval = 1;
	bool debug_beam_stop = false;
	bool debug_beam_early_stop = false;
	bool debug_skip_outside = false;
	bool debug_skip_profile = false;
	bool debug_multi_energy = false;
	int debug_multi_i = -1;
	int debug_multi_j = -1;
	bool debug_multi_reported = false;
	bool debug_outer_alpha_log = false;
	int debug_outer_alpha_hits = 0;
	int debug_outer_alpha_max = 50;
	bool debug_outer_dp_inside = false;
	int debug_outer_dp_inside_i0 = -1;
	int debug_outer_dp_inside_i1 = -1;
